...
import { HeroImage } from "../landingPage/ui/heroImage--fullwidth";
...
export const HeroUnitComponent: FC<Props> = (props) => {
  ...
  function bannerContent() {
    ...
    if (
      ...
      return (
        <div
          className="comp_hero-unit py-1 px-3 w-full flex justify-center flex-col items-center"
          {...createItemSmartLink(props.item.system.id, props.item.system.name)}
          {...createElementSmartLink(
            contentTypes.hero_unit.elements.content.codename
          )}
        >
          <div className="hidden md:block m-0 text-xl font-medium break-words hyphens-none text-white text-center w-full lg:w-1/2 pb-2 max-w-5xl">
            <RichTextElement
