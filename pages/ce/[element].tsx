...
const CustomElementTest: NextPage<IProps> = ({ elementComponent }) => {
  ...
  let customElement = (
    <div>
      <p>There was an issue loading the Custom Element</p>
    </div>
  );
  if (element && context) {
    switch (elementComponent) {
      ...
      default:
        customElement = (
          <div>
            <p>Custom element no configured in code</p>
          </div>
        );
        break;
    }
    ...
    customElement = (
      <div>
        <p>
          The <code>Element</code> and/or <code>Context</code> is not loaded yet
        </p>
      </div>
    );
  }
...
export const getStaticPaths: GetStaticPaths = async (params) => {
  return {
    paths: ["/ce/hubspotforms", "/ce/export", "/ce/read-only"],
    fallback: false,
  };
};
